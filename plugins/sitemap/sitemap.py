...
from pelican.utils import get_date, SafeDatetime
...
class SitemapGenerator(object):
    ...
    def __init__(self, context, settings, path, theme, output_path, *null):
        ...
        if not isinstance(config, dict):
            ...
            self.extraFiles = config.get('extra_files', [])
    ...
    def write_url(self, page, fd):
        ...
        if not os.path.exists(page_path) and hasattr(page, 'pagetype') and page.pagetype == 'file':
			page_path = os.path.join(self.input_path, page.save_as)
        ...
        if isinstance(page, contents.Article):
            ...
            if page.url == 'index.html' or len(page.url) == 2:
                pri = self.priorities['main']
                chfreq = self.changefreqs['main']
            else:
                pri = self.priorities['pages']
                chfreq = self.changefreqs['pages']
        elif hasattr(page, 'pagetype') and page.pagetype == 'file':
            pri = self.priorities['files']
            chfreq = self.changefreqs['files']
        else:
    ...
    def generate_output(self, writer):
        ...
        with open(path, 'w', encoding='utf-8') as fd:
            ...
            for path in self.extraFiles:
                file_path = os.path.join(self.input_path, path)
                if os.path.exists(file_path):
                    date = SafeDatetime.fromtimestamp(stat(file_path).st_mtime)
                    fake = FakePage(pagetype='file',
                                    status='published',
                                    date=date,
                                    url=path,
                                    save_as=path)
